...
def LocalbinaryPattern(imdata):
    ...
    #for debugging
    matrix = np.double(np.random.randint(10000, size=(32, 128)))
    print (matrix.shape, np.max(matrix),np.min(matrix),np.mean(matrix))
    print matrix.dtype
    feats = mahotas.features.lbp(matrix,1,8,False)
    
    print p1.dtype
    print (p1.shape, np.max(p1),np.min(p1),np.mean(p1))
