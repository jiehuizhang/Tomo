...
CThinPlateSpline* __stdcall new_CThinPlateSpline(const std::vector<Point>& pS, const std::vector<Point>& pD)
{
	return new CThinPlateSpline(pS, pD);
}

void __stdcall del_CThinPlateSpline(CThinPlateSpline *tpspline)
{
    tpspline->~CThinPlateSpline();
}

void __stdcall warpImage(CThinPlateSpline *tpspline, 
		const Mat& src, Mat& dst, float lambda, const int interpolation,const TPS_INTERPOLATION tpsInter)
{
	tpspline->warpImage(src, dst, lambda, interpolation, tpsInter);
}
*/
...
void CThinPlateSpline::warpImage(const Mat& src, Mat& dst, float lambda = 0.01, const int interpolation = INTER_CUBIC,const TPS_INTERPOLATION tpsInter = BACK_WARP)
{
        Size size = src.size();
